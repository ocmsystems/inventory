    ...
    */
    ...
    public static function listing($input){
        ...
        if(!empty($input)){
            ...
            $cols = ['transaction_number', 'prepared_by', 'destination_warehouse_id', 'datetime_prepared', 'status'];
            $orderby = $cols[0] . " DESC";

            $query = Returns::with('destination_warehouse')
    ...
    public function details(){
        return $this->hasMany('App\Models\ReturnDetails', 'return_id', 'id');
    }


    public function link(){
        $link = route('.returns.show',array($this->id));
