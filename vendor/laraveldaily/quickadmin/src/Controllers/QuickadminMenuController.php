...
use Laraveldaily\Quickadmin\Builders\ParentBuilder;
...
class QuickadminMenuController extends Controller
{
    ...
    public function rearrange(Request $request)
    {
        ...
        $menusList = Menu::with(['children'])
                        ->selectRaw('id, position, parent_id, menu_type, icon, name, title')
                        ->where('menu_type', '!=', 0)
                        ->where('parent_id', null)
                        ->orderBy('position')->get();
                ...
                foreach ($menu->children as $child) {
                    if($child->children()->first() == null){
                        ...
                        foreach ($child->children as $grandchild) {
                            $grandchild->position  = $request->{'grandchild-' . $grandchild->id};

                            // p($grandchild->title . " = " .$grandchild->position);
                            $grandchild->save();
                        }
    ...
    public function createCrud()
    {
        ...
        $roles             = Role::all();
        $parentsSelect = Menu::parentSelectArray(true);
...
//        dd($fields);
        ...
        $modelBuilder->build($request->parent_id);
        // Create request
        $requestBuilder = new RequestBuilder();
        $requestBuilder->build();
        // Create controller
        $controllerBuilder = new ControllerBuilder();
        $controllerBuilder->build($request->parent_id);
        // Create views
        $viewsBuilder = new ViewsBuilder();
        $viewsBuilder->build($request->parent_id);
    ...
    {
        $roles = Role::all();
        $parentsSelect = Menu::parentSelectArray();
        
        return view('qa::menus.createParent', compact('roles', 'parentsSelect'));
    }
    ...
    {   
        ...
        $parentBuilder->buildParent($request->title, $request->parent_id);
    ...
    public function insertCustom(Request $request)
    {
        ...
        // Create controller
        $controllerBuilder = new ControllerBuilder();
        $controllerBuilder->buildCustom($request->name, $request->parent_id);

        // Create views
        $viewsBuilder = new ViewsBuilder();
        $viewsBuilder->buildCustom($request->name, $request->parent_id);
